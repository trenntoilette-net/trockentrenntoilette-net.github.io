<template>
  <div>
    <div class="sidebar-widget schedule-widget mt-5">
      <h5 class="mb-4">{{ product.name }}</h5>
      <ul class="list-unstyled">
        <li class="d-flex justify-content-between align-items-center">
          <a href="#">Marke</a> <span>{{ product.brand }}</span>
        </li>
        <hr />
        <li class="d-flex justify-content-between align-items-center">
          <a href="#">Material</a>
          <span>{{ product.material }}</span>
        </li>
        <hr />
        <li class="d-flex justify-content-between align-items-center">
          <a href="#">Maße</a>
          <span>{{ product.size }}</span>
        </li>
        <hr />
        <li class="d-flex justify-content-between align-items-center">
          <a href="#">Gewicht</a> <span>{{ product.weight }}</span>
        </li>
        <hr />
        <li class="d-flex justify-content-between align-items-center">
          <a href="#">Traglast</a> <span>{{ product.payload }}</span>
        </li>
        <hr />
        <li class="d-flex justify-content-between align-items-center">
          <a href="#">Funktion</a> <span>{{ product.function }}</span>
        </li>
        <hr />
        <li class="d-flex justify-content-between align-items-center">
          <a href="#">Bewertung</a>
          <div class="mb-1 mt-3">
            <small
              v-for="star in product.stars"
              :key="star"
              class="pl-3 fa fa-star text-primary"
            ></small>
            <small
              v-for="index in 5 - product.stars"
              :key="index"
              class="fa fa-star text-default"
            ></small>
          </div>
        </li>
        <hr />
      </ul>
      <div class="sidebar-contact-info mt-4">
        <p class="mb-0">Preis</p>
        <h3>{{ product.price }} EUR</h3>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "productcard",
  props: {
    product: Object,
  },
};
</script>
